<template>
  <div>
    <el-card>
      ...
      <GoodsManageEdit v-if="showEdit" :item="item" @close="closeDialog"></GoodsManageEdit>
...
import GoodsManageEdit from './GoodsManageEdit'
export default {
  components: {
    GoodsManageEdit
  },
  ...
  methods: {
    ...
    getData() {
      ...
      //   this.selectDate = []
      // }
      this.$http.get('/manage/product/lists').then(res => {
    ...
    deleteGood(data) {
      this.$confirm('确定删除该产品?', '提示', {
        ...
        .then(res => {
          this.$http
            .post('/manage/product/del', { id: data.product_id })
            .then(res => {
              this.getData()
            })
            .catch(err => {})
