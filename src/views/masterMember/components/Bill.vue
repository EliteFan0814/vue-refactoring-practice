    ...
    <el-tabs v-model="activeName" @tab-click="getTable">
      <el-tab-pane v-for="(item,index) in classlist" :key="index" :label="`${item.label}记录`" :name="item.value">
        <el-table :data="tableData" style="width: 100%">
          ...
          <el-table-column prop="type_str" align="center" label="账户类型">
          </el-table-column>
          <el-table-column prop="remark" align="center" label="账单说明">
          </el-table-column>
        </el-table>
      </el-tab-pane>
    </el-tabs>
...
export default {
  ...
  data() {
    return {
      ...
      activeName: 'tx',
      tableData: [],
      tableUrl: '/manage/worker_bill/lists',
      chargeWay: '',
    ...
    getData() {
      ...
      this.$http
        ...
        .then(res => {
          ...
          this.classlist = res.data.class_screen
        })
    },
    filterData() {
